...
class NGio
{
			...
			.addOutcomeHandler(function(outcome)
			{
				switch (outcome)
				{
					case SUCCESS(data):
						...
						callback(GAME_VER);
					case FAIL(error):
						trace('failed to check NG version: ' + error);
				}
			})
			.send();
	}
	...
	static public function init()
	{
		...
		trace("connecting to newgrounds");
			...
			function onSessionFail(outcome:LoginOutcome)
			{
	...
	static public function login(?popupLauncher:(Void->Void)->Void, onComplete:ConnectionResult->Void)
	{
		...
		var passportHandler:String->Void = null;
		if (popupLauncher != null)
		{
			passportHandler = function (url:String) popupLauncher(NG.core.openPassportUrl);
		}

		NG.core.requestLogin(function (outcome:LoginOutcome)
		{
			switch (outcome)
			{
				case SUCCESS:
				...
				case FAIL(ERROR(error)):
					if (onComplete != null)
						onComplete(Fail(error.toString()));
			}
		}, passportHandler);
	}
